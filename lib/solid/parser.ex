defmodule Solid.Parser do
  ...
  single_quoted_string = ignore(string(~s(')))
                         |> repeat(lookahead_not(ascii_char([?']))
                         |> choice([string(~s(\')), utf8_char([])]))
                         |> ignore(string(~s(')))
                         |> reduce({List, :to_string, []})

  double_quoted_string = ignore(string(~s(")))
                         |> repeat(lookahead_not(ascii_char([?"]))
                         |> choice([string(~s(\")), utf8_char([])]))
                         |> ignore(string(~s(")))
                         |> reduce({List, :to_string, []})
  ...
  value = choice([float, int, true_value, false_value, null, single_quoted_string, double_quoted_string])
  ...
  filter_name = ascii_string([?a..?z, ?A..?Z], 1)
  ...
  comment_tag = ignore(opening_tag)
                |> ignore(space)
                |> ignore(comment)
                |> ignore(space)
                |> ignore(closing_tag)
  ...
  counter_tag = ignore(opening_tag)
                |> ignore(space)
  ...
  cond_case_tag = tag(case_tag, :case_exp)
                  |> ignore(parsec(:liquid_entry)) # FIXME
  ...
  operator = choice([string("=="), string("!="), string(">="), string("<="), string(">"), string("<"), string("contains")])
             |> map({:erlang, :binary_to_existing_atom, [:utf8]})

  boolean_operation = tag(argument, :arg1)
                      |> ignore(space)
                      |> tag(operator, :op)
                      |> ignore(space)
                      |> tag(argument, :arg2)
  ...
  boolean_expression = expression
                       |> repeat(choice([bool_and, bool_or])
                       |> concat(expression) |> wrap())

  if_tag = ignore(opening_tag)
           |> ignore(space)
           |> ignore(string("if"))
  ...
  cond_unless_tag = tag(unless_tag, :unless_exp)
                    |> tag(times(elsif_tag, min: 0), :elsif_exps)
                    |> optional(tag(else_tag, :else_exp))
                    |> ignore(opening_tag)
                    |> ignore(space)
  ...
  tags = choice([counter_tag, comment_tag, assign_tag, cond_if_tag, cond_unless_tag, cond_case_tag, for_tag])
         |> tag(:tag)
